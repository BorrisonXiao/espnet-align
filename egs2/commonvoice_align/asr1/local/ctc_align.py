...
def ctc_align(
    ...
    **kwargs,
    ...
    for line in wav_scp:
        segid, audio = line.strip().split()
        # load text file
        transcripts = text.readline().strip().split(maxsplit=1)[1]
        transcripts = transcripts.split()
        transcripts = [f"utt{i} {c}" for i, c in enumerate(transcripts)]
        output = os.path.join(output_dir, f"{segid}.txt")
...
def get_parser():
    ...
    group.add_argument(
        ...
        " The mapping of uttid to the one-line text",
    )
    group.add_argument(
        "--output_dir",
        type=Path,
        default="-",
        help="Output in the form of `segments` files."
    )
    return parser
