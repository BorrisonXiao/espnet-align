...
def inference(
    ...
    scp_map: Path,
    ngram_dir: Optional[Path],
    inference_ngram: Optional[str],
    ...
    with open(scp_map, "r") as f:
        for line in f:
            segid, scpid = line.strip().split()
            scpid2segid[scpid] = segid

    # Load ngram LMs
    mlevel_ngram_dir = os.path.join(ngram_dir, "mlevel")
    slevel_ngram_dir = os.path.join(ngram_dir, "slevel")

    # Load the meeting level ngram LMs
    ngrams_mlevel = {}
    for mid in os.listdir(mlevel_ngram_dir):
        ngrams_mlevel[mid] = os.path.join(mlevel_ngram_dir, mid, inference_ngram)

    # Load the segment level ngram LMs
    ngrams_slevel = {}
    for scpid in os.listdir(slevel_ngram_dir):
        if scpid in scpid2segid:
            ngrams_slevel[scpid2segid[scpid]] = os.path.join(
                slevel_ngram_dir, scpid, inference_ngram)

    # 1. Set random-seed
    set_all_random_seed(seed)

    # 2. Build speech2text
    # Randomly create an init_ngram model
    init_ngram = list(ngrams_slevel.keys())[0]
    init_ngram_file = ngrams_slevel[init_ngram]
    ...
    speech2text = (list(ngrams_slevel.keys())[0], Speech2Text.from_pretrained(
        model_tag=model_tag,
        **speech2text_kwargs,
    ))
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            try:
                # On-demand rebuild of speech2text object to use the correct ngram model
                # Note that this one-item-cache-like strategy makes the order of keys CRUCIAL
                segid = keys[0]
                batch_mid = extract_mid(segid)
                if speech2text[0] != segid and speech2text[0] != batch_mid:
                    if segid in ngrams_slevel:
                        logging.info(
                            f"Loading segment-level ngram model {ngrams_slevel[segid]}...")
                        speech2text_kwargs["ngram_file"] = ngrams_slevel[segid]
                        model_name = segid
                    else:
                        logging.info(
                            f"Segment level ngram model not found for {segid}, loading meeting-level ngram model {ngrams_mlevel[batch_mid]}...")
                        speech2text_kwargs["ngram_file"] = ngrams_mlevel[batch_mid]
                        model_name = batch_mid
                    speech2text = (model_name, Speech2Text.from_pretrained(
                        model_tag=model_tag,
                        **speech2text_kwargs,
                    ))
...
def get_parser():
    ...
    parser.add_argument("--scp_map", type=Path, required=True,
                        help="The scp_map file mapping the uttid to segid")
    ...
    group.add_argument(
        "--ngram_dir",
        type=Path,
        help="N-gram parameter directory",
    )
    group.add_argument(
        "--inference_ngram",
        type=str,
        help="N-gram model used for inference",
    )
    group.add_argument(
        "--model_tag",
        type=str,
